...
import {
  ...
  EditPurchaseModal,
  PurchaseViewModal,
} from "@/components/purchase-modals";
import {
  createPurchase,
  deletePurchase,
  getPurchases,
...
export default function Purchases() {
    ...
    (async () => {
      try {
        const [vendorsResult, purchasesResult] = await Promise.all([
          ...
          getPurchases({ cache: "no-store" })
            .then((data) => ({ ok: true as const, data }))
            .catch(() => ({ ok: false as const, data: [] as Purchase[] })),
        ]);

        if (!vendorsResult.ok || !purchasesResult.ok) {
          toast({
  ...
  const handleUpdate = async () => {
    ...
    try {
      const updated = await updatePurchase(editingPurchase.id, {
